...
pub use coverage::{FragmentDataIter, ContactDataCounter, CountingStrategy, FragmentData, BaseData};
pub use genome::{
    read_transcripts_from_gff, read_transcripts_from_gtf, ChromSizes, ChromValueIter, ChromValues,
    FeatureCounter, GeneCount, GenomeBaseIndex, Promoters, Transcript, TranscriptCount,
...
impl<B: Backend> SnapData for AnnData<B> {
    ...
    fn fragment_size_distribution(&self, max_size: usize) -> Result<Vec<usize>> {
        if let Some(fragment) = self.obsm().get_item_iter("fragment_paired", 500) {
            Ok(qc::fragment_size_distribution(
                fragment.map(|x| x.0),
                max_size,
            ))
        } else {
